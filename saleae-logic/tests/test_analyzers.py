...
async def test_add_analyzer(server, capture_with_data):
    ...
    result = await server.call_tool("add_analyzer", {
...
async def test_export_analyzer_data(server, capture_with_data):
    ...
    idx = json.loads(result[0].text)["analyzer_index"]

    # Export
    result = await server.call_tool("export_analyzer_data", {
        "capture_id": capture_with_data,
        "analyzer_index": idx,
        "radix": "hex",
    })
...
async def test_export_raw_data(server, capture_with_data):
    ...
    result = await server.call_tool("export_raw_data", {
        "capture_id": capture_with_data,
