...
impl EmbeddedDebuggerToolHandler {
    ...
    // =============================================================================
    // Boot Validation Tool
    // =============================================================================

    #[tool(description = "Flash firmware and validate boot via RTT pattern matching")]
    async fn validate_boot(&self, Parameters(args): Parameters<ValidateBootArgs>) -> Result<CallToolResult, McpError> {
        debug!("Validating boot for session: {}, pattern: {}", args.session_id, args.success_pattern);
        ...
        let session_arc = {
            ...
            match sessions.get(&args.session_id) {
                ...
                None => {
                    return Err(McpError::internal_error(
                        format!("Session '{}' not found", args.session_id), None
                    ));
                }
            }
        };
        ...
        info!("validate_boot: Flashing firmware {}", args.file_path);
        {
            ...
            if let Err(e) = crate::flash::FlashManager::program_file(
                ...
                crate::flash::FileFormat::Auto,
                None
            ).await {
                errors.push(format!("Flash failed: {}", e));
        ...
        info!("validate_boot: Resetting target");
        {
            ...
            if let Err(e) = core.reset() {
                errors.push(format!("Reset failed: {}", e));
            }
            if let Err(e) = core.run() {
                errors.push(format!("Run failed: {}", e));
            }
        }

        // Step 3: Wait for boot, then attach RTT
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        ...
        // Step 4: Monitor RTT for success pattern
        let pattern = regex::Regex::new(&args.success_pattern).unwrap_or_else(|_| {
            // Fall back to literal string matching
            regex::Regex::new(&regex::escape(&args.success_pattern)).unwrap()
        });

        let timeout = std::time::Duration::from_millis(args.timeout_ms as u64);
        ...
        while std::time::Instant::now() < deadline {
            let mut rtt_manager = session_arc.rtt_manager.lock().await;
            if rtt_manager.is_attached() {
                if let Ok(data) = rtt_manager.read_channel(args.rtt_channel).await {
                    if let Ok(text) = String::from_utf8(data) {
                        rtt_output.push_str(&text);

                        // Check for pattern match
                        if let Some(m) = pattern.find(&rtt_output) {
        ...
        let success = matched_pattern.is_some();
        ...
        let message = if success {
            format!(
                ...
                boot_time_ms,
                if args.capture_output && !rtt_output.is_empty() {
                    format!("\nRTT Output:\n{}", rtt_output)
                } else { String::new() }
            )
        } else {
            format!(
                ...
                if args.capture_output && !rtt_output.is_empty() {
                    format!("\nRTT Output:\n{}", rtt_output)
                } else { "\nNo RTT output captured".to_string() }
            )
        };

        info!("Boot validation completed: success={}, time={}ms", success, boot_time_ms);
        Ok(CallToolResult::success(vec![Content::text(message)]))
    }
    ...
    #[tool(description = "Flash ESP32 via esptool (for Xtensa chips: ESP32, ESP32-S2, ESP32-S3)")]
    async fn esptool_flash(&self, Parameters(args): Parameters<EsptoolFlashArgs>) -> Result<CallToolResult, McpError> {
        info!("esptool_flash: port={}, chip={}, file={}", args.port, args.chip, args.file_path);

        // Build esptool command
        let mut cmd = std::process::Command::new("esptool.py");
        cmd.args(["--chip", &args.chip])
           .args(["--port", &args.port])
           .args(["--baud", &args.baud_rate.to_string()]);

        if !args.reset_after {
            cmd.arg("--after=no_reset");
        }
        ...
        cmd.args([&args.address, &args.file_path]);
            ...
            Ok(output) => {
                ...
                if output.status.success() {
                    let message = format!(
                        ...
                        Duration: {:.1}s\n\n\
                        Output:\n{}",
                        args.port, args.chip, args.file_path, args.address,
                        elapsed.as_secs_f64(),
                        stdout
                    );
                    Ok(CallToolResult::success(vec![Content::text(message)]))
                    ...
                    let message = format!(
                        "❌ esptool flash failed\n\n\
                        Port: {}\n\
                        Chip: {}\n\
                        File: {}\n\n\
                        Error:\n{}\n{}",
                        args.port, args.chip, args.file_path,
                        stderr, stdout
                    );
    ...
    #[tool(description = "Read serial output from ESP32 (monitor mode)")]
    async fn esptool_monitor(&self, Parameters(args): Parameters<EsptoolMonitorArgs>) -> Result<CallToolResult, McpError> {
        info!("esptool_monitor: port={}, baud={}", args.port, args.baud_rate);

        // Use esptool.py monitor or a simple serial read
        // For simplicity, use Python to read serial
        let script = format!(
            r#"
import serial
import sys
...
"#,
            ...
            Ok(output) => {
                let data = if output.stdout.is_empty() {
                    ...
                    match String::from_utf8(output.stdout.clone()) {
                        Ok(text) => text,
                        Err(_) => format!("Binary data ({} bytes): {}",
                            output.stdout.len(),
                            hex::encode(&output.stdout[..std::cmp::min(64, output.stdout.len())])
                        )
                    }
                };
                ...
                if output.status.success() {
                    let message = format!(
                        ...
                        Bytes: {}\n\n\
                        Data:\n{}",
                        args.port, args.baud_rate, output.stdout.len(), data
                    );
                    Ok(CallToolResult::success(vec![Content::text(message)]))
                } else {
                    let stderr = String::from_utf8_lossy(&output.stderr);
                    Err(McpError::internal_error(
                        format!("Serial read failed: {}", stderr), None
                    ))
                }
            }
            Err(e) => {
                Err(McpError::internal_error(
                    format!("Failed to run serial monitor: {}\n\nMake sure pyserial is installed: pip install pyserial", e),
                    None
                ))
            }
        }
    ...
    // =============================================================================
    // Vendor Tool: nrfjprog
    // =============================================================================

    #[tool(description = "Flash Nordic device via nrfjprog")]
    async fn nrfjprog_flash(&self, Parameters(args): Parameters<NrfjprogFlashArgs>) -> Result<CallToolResult, McpError> {
        info!("nrfjprog_flash: family={}, file={}", args.family, args.file_path);

        // Build nrfjprog command
        let mut cmd = std::process::Command::new("nrfjprog");
        cmd.args(["--family", &args.family]);
        ...
        match cmd.output() {
            Ok(output) => {
                ...
                if !output.status.success() {
                    return Err(McpError::internal_error(
                        format!("nrfjprog program failed:\n{}\n{}", stderr, stdout), None
                    ));
                }

                // Reset if requested
                if args.reset_after {
                    let reset_output = std::process::Command::new("nrfjprog")
                        .args(["--family", &args.family])
                        .args(if let Some(snr) = &args.snr { vec!["--snr", snr] } else { vec![] })
                        .arg("--reset")
                        .output();

                    if let Err(e) = reset_output {
                        warn!("nrfjprog reset failed: {}", e);
                    }
                }

                let message = format!(
                    "✅ nrfjprog flash completed!\n\n\
                    Family: {}\n\
                    File: {}\n\
                    Erase: {}\n\
                    Verify: {}\n\
                    ...
                    Duration: {:.1}s\n\n\
                    Output:\n{}",
                    args.family, args.file_path,
                    if args.sectorerase { "sector" } else { "chip" },
            ...
            Err(e) => {
                Err(McpError::internal_error(
                    format!("Failed to run nrfjprog: {}\n\nMake sure nRF Command Line Tools are installed", e),
                    None
                ))
            }
        }
    ...
    #[tool(description = "Load custom target definition from YAML file")]
    async fn load_custom_target(&self, Parameters(args): Parameters<LoadCustomTargetArgs>) -> Result<CallToolResult, McpError> {
        info!("Loading custom target from: {}", args.target_file_path);

        // Open the target file as a reader
        let file = match std::fs::File::open(&args.target_file_path) {
            Ok(f) => f,
            Err(e) => {
                return Err(McpError::internal_error(
                    format!("Failed to open target file '{}': {}", args.target_file_path, e),
                    None
                ));
            }
        };
        ...
        // Load into probe-rs
        match probe_rs::config::add_target_from_yaml(file) {
            Ok(name) => {
                let message = format!(
                    "✅ Custom target loaded successfully!\n\n\
                    File: {}\n\
                    Target name: {}\n\n\
                    You can now use this target with the connect tool.",
                    args.target_file_path, name
                );
                info!("Loaded custom target: {}", name);
                Ok(CallToolResult::success(vec![Content::text(message)]))
            }
            Err(e) => {
                Err(McpError::internal_error(
                    format!("Failed to load custom target: {}\n\nEnsure the YAML follows probe-rs target format", e),
                    None
                ))
            }
        }
